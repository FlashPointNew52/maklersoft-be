...
public class Authorisation {
    ...
    public Authorisation() {
        this.makl_user.setAccountId(1556581348163L); //old_1545092165300l
        setupEndpoints();
    }
    ...
    private void setupEndpoints() {
        ...
        before((request, response) -> {
            ...
            if (AUTH_CHECK_DISABLED || !request.uri().startsWith("/api") || request.requestMethod().equals("OPTIONS")) return;
            ...
            if(!AppConfig.AUTORISATION_WHITELIST.contains(request.headers("Authorization"))){
                if (request.session().isNew() || request.session().attribute("logged_in") == null || (boolean)request.session().attribute("logged_in") != true) {
                    halt(401, "Время вашей сессии истекло. Пожалуйста, авторизуйтесь!");
                } else{
        ...
        post("/session/login", (request, res) -> {
            ...
            User user = App.userService.getByPhone(new ArrayList<String>() {{ add("7" + phone);}});
            if(user != null) {
        ...
        post("/session/registrate", (request, response) -> {
            ...
            try {
                ...
                if (phone.length() < 10) {
                    response.status(400);
                    return new ErrorMsg("001:Wrong format phone", "Неверный формат телефона", "RegError", 0);
                }
                ...
                List<ErrorMsg> errors = App.userService.check(new_user);

                if (errors.size() == 0) {
                    Organisation org = new Organisation();
                    org.setName(org_name);
                    org.setOurCompany(true);
                    ...
                    if (errors.size() == 0) {
                        ...
                        new_user.setDepartment("management");
                        new_user.setOrganisationId(org.getId());
                        new_user.setStateCode("active");
                ...
                response.status(400);
                return new ErrorMsg("900: System error | " + ExceptionUtils.getStackTrace(exp), "Ошибка при регистрации", "RegError", 0);
            }
            return result;
    ...
    private boolean sendSms(String phone, String text){
        Smsc sms = new Smsc();
        try {
            String[] ret = sms.send_sms(PhoneBlock.normalisePhone(phone), text, 0, "", "", 0, "MaklerSoft", "");
        } catch (Exception exp) {
            return false;
        }
        return true;
